...
module ``about the stock example`` =
    ...
    let parse (x: string array) =
        let date = DateTime.Parse(x.[0])
        let open' = Double.Parse(x.[1])
        let close = Double.Parse(x.[4])

        date, Math.Abs(open' - close)

    let findGreatest (x: string list) =
        x
        |> List.skip 1
        |> List.map (splitCommas >> parse)
        |> List.maxBy snd
        |> fst
    ...
    let YouGotTheAnswerCorrect () =
        let result = findGreatest stockData

        AssertEquality "2012-03-13" (result.ToString("yyyy-MM-dd"))
